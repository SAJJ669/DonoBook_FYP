...
import {
  ArrowLeft, MessageSquare, Gift, RefreshCw, Package, MapPin, X,
  ChevronLeft, ChevronRight, Tag, Palette, Ruler, User, FileText, Share2
} from "lucide-react";
...
const ItemDetails = () => {
    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-4 py-16 flex flex-col items-center gap-4">
          ...
          <p className="text-muted-foreground text-lg">Loading item details...</p>
        </div>
      </div>
    );
  }
  ...
  const getImages = () => {
    ...
    if (typeof item.image_url === 'string' && item.image_url.startsWith('[')) {
      try { return JSON.parse(item.image_url); } catch { return [item.image_url]; }
    }
  ...
  const conditionColor = CONDITION_COLORS[item.condition] || CONDITION_COLORS.used;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-2">
            <div className="sticky top-24">
                      ...
                      <CarouselItem key={index}>
                        ...
                        >
                          <img
                            ...
                            className={`w-full h-full object-cover transition-all duration-300 ${item.status !== "available" ? "grayscale opacity-60" : ""}`}
                          />
              ...
              <p className="text-xs text-center text-muted-foreground mt-2">Tap image to zoom in</p>
            </div>
          </div>
          ...
          <div className="lg:col-span-3 space-y-5">
            ...
            <div className="flex flex-wrap gap-2">
              <Badge
                className={`gap-1.5 text-sm py-1.5 px-3 ${item.type === "donate"
            ...
            {/* Item metadata */}
            {(item.brand || item.color || item.size || item.material) && (
              <Card className="shadow-card border-border">
                <CardContent className="p-4">
                  <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Item Details</h3>
            ...
            {!isOwner ? (
              <Button
                ...
                size="lg"
                className={`w-full gap-2 h-12 text-base font-semibold btn-glow transition-smooth ${item.status === "available"
